...
object SparkStreaming {
  ...
  def defineApp(props: Properties, ssc: StreamingContext) {
    ...
    dstream.foreachRDD { rdd =>
      ...
      if (!rdd.isEmpty()) {
        println("LOOP1::RDD-1:" + rdd) //LOOP1::RDD-1:KafkaRDD
        //ConsumerRecord(topic = tp_in_profileupdates, partition = 0, offset = 108, CreateTime = 1586555097059, serialized key size = 7, serialized val
        //ue size = 153, headers = RecordHeaders(headers = [], isReadOnly = false), key = user.id, value = {"user_fname":"sunil", "user_lname":"miri",
        //"user_id":"sunil.miri", "updated_at":"2020-03-30 14:15:16.000", "modified_fields":[{"address":"new value"}]} )
        rdd.foreach(println)
        println("LOOP1::Offsets:")
        rdd.foreachPartition { iter =>
          val o: OffsetRange = offsetRanges(TaskContext.get.partitionId)
          //tp_in_profileupdates 0 108 109
          println(s"${o.topic} ${o.partition} ${o.fromOffset} ${o.untilOffset}")
        }
        //each rdd is a record of instance: ConsumerRecord. So get value of it as string. then parse as json in to new DF
        val rddValAsStr = rdd.map(record => (record.value()).toString)
        println("LOOP1::rddValAsStr:" + rddValAsStr) //LOOP1::rddValAsStr:MapPartitionsRDD
        val rddValDF = sqlContext.read.json(rddValAsStr) //LOOP1::rddValDF:[modified_fields: array<struct<address:string>>
        println("LOOP1::rddValDF:" + rddValDF)
        rddValDF.printSchema()
        rddValDF.show(2)
        /**
         * root
         ...
         * +---------------+--------------------+----------+-----------+----------+
         * |modified_fields|          updated_at|user_fname|    user_id|user_lname|
         * +---------------+--------------------+----------+-----------+----------+
         * |  [[new value]]|2020-03-30 14:15:...|     sunil|sunil.miri2|      miri|
         * +---------------+--------------------+----------+-----------+----------+
         */
        val enrichedUDF: DataFrame = addDatePartition(dynamicPartCols, rddValDF)
        ...
        println("LOOP1::Completed");
      }
      //save the offsets once entire operation is completed
      println("LOOP1::Saving Offsets");
